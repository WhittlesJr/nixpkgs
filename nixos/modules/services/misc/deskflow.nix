...
let
  ...
  cfgC = config.services.deskflow.client;
  cfgS = config.services.deskflow.server;
...

{
  ...
  options = {
    ...
    services.deskflow = {
      ...
      client = {
        enable = lib.mkEnableOption "the Deskflow client (receive keyboard and mouse events from a Deskflow server)";
      ...
      server = {
        enable = lib.mkEnableOption "the Deskflow server (send keyboard and mouse events)";

        configFile = lib.mkOption {
          type = lib.types.path;
          default = "/etc/deskflow-server.conf";
          description = "The Deskflow server configuration file.";
        };
        screenName = lib.mkOption {
          type = lib.types.str;
          default = "";
        ...
        tls = {
          enable = lib.mkOption {
            ...
            description = ''
              ...
              generated by starting the Deskflow GUI once and entering
              a valid product key.
            '';
          };
          ...
          cert = lib.mkOption {
            ...
            default = null;
            example = "~/.deskflow/SSL/Deskflow.pem";
  ...
  config = lib.mkMerge [
    (lib.mkIf cfgC.enable {
      systemd.user.services.deskflow-client = {
        ...
        path = [ pkgs.deskflow ];
        serviceConfig.ExecStart = ''${pkgs.deskflow}/bin/deskflow-client -f ${
    ...
    (lib.mkIf cfgS.enable {
      systemd.user.services.deskflow-server = {
        ...
        path = [ pkgs.deskflow ];
        serviceConfig.ExecStart = ''${pkgs.deskflow}/bin/deskflow-server -c ${cfgS.configFile} -f${
...
/*
  DESKFLOW SERVER example configuration file
  section: screens
    laptop:
    dm:
    win:
